...
hierarchy,tup = Utilfn.hierarchical_cluster(x,verbose = True)
hierarchy2, tup2 = Utilfn.hierarchical_cluster_balanced(x,verbose=True)

#extends lines to clean plot
for i in range(0,len(hierarchy)):
    hierarchy[i,2] = hierarchy[i,2]+max([tup[int(hierarchy[i,0])][2],tup[int(hierarchy[i,1])][2]])
for i in range(0,len(hierarchy2)):
    hierarchy2[i,2] = hierarchy2[i,2]+max([tup2[int(hierarchy2[i,0])][2],tup2[int(hierarchy2[i,1])][2]])


# plot dendrogram with interactive coloring
Utilfn.plot_dendrogram(hierarchy2,names,group_dist)
